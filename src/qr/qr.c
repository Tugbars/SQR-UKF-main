...
//==============================================================================
// AVX2 OPTIMIZED TRANSPOSE FUNCTIONS
//==============================================================================
 ...
 */
static void transpose_matrix_avx2_blocked(
    float *restrict dst,
    ...
    uint16_t ld_src, uint16_t ld_dst)
{
    ...
    for (; i + 8 <= rows; i += 8)
    {
        ...
        for (; j + 8 <= cols; j += 8)
        {
            ...
            // Transpose 2×2 blocks using unpacklo/unpackhi
            ...
            // Transpose 4×4 blocks using shuffle
            ...
            // Final 8×8 transpose using cross-lane permute
 ...
 * @param ld_src Leading dimension of source
 */
static void copy_strided_to_contiguous_opt(
    float *restrict dst,
    const float *restrict src,
    uint16_t rows, uint16_t cols,
    uint16_t ld_src)
 ...
 *   3. Build T_left from Y_left and tau_left
    ...
    uint16_t threshold)
{
    ...
    float *Z = T_left + t_left_size;
    float *Z_temp = Z + z_size;
    float *YT = Z_temp + z_size;
    float *workspace_next = YT + yt_size;
    ...
    //==========================================================================
    // STEP 3: Apply block reflector to right columns (LEVEL 3 BLAS!)
    //==========================================================================
                           ...
                           uint16_t ldy)
{
    ...
    for (uint16_t i = 0; i < ib; ++i)
    {
        ...
        for (uint16_t j = 0; j < i; ++j)
        {
...
#endif
            {
                ...
                for (uint16_t r = 0; r < m; ++r)
                    dot += (double)Y[r * ldy + i] * (double)Y[r * ldy + j];
            }
            w[j] = -(double)tau[i] * dot;
        }

        //======================================================================
        // Compute T[i, 0:i-1] = w * T[0:i-1, 0:i-1]
        ...
        {
            double sum = 0.0;
            // T is lower triangular, so T[k, j] is non-zero only for k >= j
            for (uint16_t k = j; k < i; ++k)
                sum += w[k] * (double)T[k * ib + j];
            T[i * ib + j] = (float)sum;
        }
    }
    ...
    float *restrict YT)
{
    ...
    copy_strided_to_contiguous_opt(Y_contig, Y, m, ib, ldy);
...
static int qr_factor_blocked(qr_workspace *ws, float *A, uint16_t m, uint16_t n)
{
    ...
    for (uint16_t k = 0; k < kmax; k += ws->ib)
    {
        ...
        if (ws->Y_stored && ws->T_stored)
        {
            ...
            // ✅ FIX: build_T_matrix uses stride=block_size, not ws->ib
        ...
        // Apply block reflector to trailing matrix: C = (I - Y*T*Y^T) * C
        // This is Level 3 BLAS (3 GEMM calls) instead of IB Level 2 calls
        if (cols_right > 0)
        {
            apply_block_reflector_strided(
                &A[k * n + (k + block_size)],  // Trailing matrix C
