...
static void apply_reflector(float *A, uint16_t m, uint16_t n,
                            uint16_t lda, // ✅ ADDED: explicit leading dimension
                            const float *v, float tau)
{
    ...
    for (uint16_t j = 0; j < n; ++j)
    {
        ...
        for (uint16_t i = 0; i < m; ++i)
            dot += v[i] * A[i * lda + j]; // ✅ FIXED: use lda, not n

        for (uint16_t i = 0; i < m; ++i)
            A[i * lda + j] -= tau * v[i] * (float)dot; // ✅ FIXED: use lda
    }
}
