    ...
    int *kernel_width)
{
    ...
    int m_class;
    if (m_height >= 16)
        m_class = 4; // 16+ rows → use 16× kernels
    else if (m_height >= 9)
        m_class = 3; // 9-15 rows → use 16× kernels (composite)
    else if (m_height >= 5)
        m_class = 2; // 5-8 rows → use 8× kernels
    else if (m_height >= 1)
        m_class = 1; // 1-4 rows → use 4× kernels
    else
        m_class = 0; // ERROR
    ...
    if (n_width >= 16)
        ...
        n_class = 3; // 9-15 cols (needs 16-wide kernel)
    else if (n_width >= 6)
        n_class = 2; // 6-8 cols
    else if (n_width >= 1)
        n_class = 1; // 1-5 cols
...
size_t gemm_workspace_query(size_t M, size_t K, size_t N)
{
    ...
    // Component sizes (corrected!)
    size_t a_size = MR * KC * sizeof(float);  // Was MC * KC
    
    size_t max_n_panels = (NC + NR - 1) / NR;
    size_t b_size = max_n_panels * KC * 16 * sizeof(float);  // Was KC * NC
